...
def is_antisymmetric(graph):
    ...
    for i in range(graph.size):
        for j in range(graph.size):
            if graph.matrix[i][j] and graph.matrix[j][i] and j != i:
                return False
    return True
...
def is_transitive(graph):
    ...
    for i in range(graph.size):
        for j in range(graph.size):
            if graph.matrix[i][j]:
                for g in range(graph.size):
                    if graph.matrix[j][g] and not graph.matrix[i][g]:
                        return False
    return True
...
def transitive_closure(graph):
    ...
    for i in range(new_graph.size):
        for j in range(new_graph.size):
            if new_graph.matrix[i][j]:
                for g in range(new_graph.size):
                    if new_graph.matrix[j][g]:
                        new_graph.matrix[i][g] = True
            if new_graph.matrix[j][i]:
                for g in range(new_graph.size):
                    if new_graph.matrix[i][g]:
                        new_graph.matrix[j][g] = True
    return new_graph
